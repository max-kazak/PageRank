...
class PageRank(object):     
    ...
    def initAllStructs(self):
        ...
        if self.sinkHandling == 0:
            # self loops for sink handling (type 1)
            for x in self.nodeIDs:
                self.adjList[x].append(x)

        # 2.
        self.inList = defaultdict(list)
        for x, ys in self.adjList.items():
            for y in ys:
                self.inList[y].append(x)

        # 3.
        # self.outDeg = defaultdict(int)
        # for x, ys in self.adjList.items():
        #     self.outDeg[x] = len(ys)
        self.outDeg = [0] * self.N
        for x, ys in self.adjList.items():
            self.outDeg[x] = len(ys)

        # 4.
        if self.sinkHandling == 1:
            self.sinkIDs = [x for x in self.nodeIDs if self.outDeg[x] == 0]

        # 5.
        self.rankVec = [1./self.N] * self.N
    ...
    def solveRankIter(self, oldRankVec):
        ...
        random_walk = (1 - alpha) / N

        if self.sinkHandling == 1:
            for x in self.sinkIDs:
                random_walk += oldRankVec[x] * alpha / N

        for x in self.nodeIDs:
            newRankVec[x] = random_walk
            for y in self.inList[x]:
                newRankVec[x] += oldRankVec[y] * alpha / outDeg[y]
    ...
    def solveRankToEps(self, eps):
        ...
        while (prevRankVec is None) or (np.max(np.abs(np.array(prevRankVec) - np.array(newRankVec))) > eps):
            prevRankVec = newRankVec
